...
public class Node: CustomStringConvertible, Equatable {
    ...
    public var articulationPoint: Bool
    public var graphId: Int
    public var parent: Node?
    
    public init(label: String, graphId: Int) {
        self.label = label
...
public class Graph: CustomStringConvertible, Equatable {
    ...
    public func addNode(_ label: String) -> Node {
        let node = Node(label: label, graphId: nodes.count)
        nodes.append(node)
        return node
    }
    ...
    public func addEdge(_ source: Node, neighbor: Node, bidirectional: Bool) {
        let edge = Edge(neighbor: neighbor)
        source.neighbors.append(edge)
        if(bidirectional) {
            let edge = Edge(neighbor: source)
            neighbor.neighbors.append(edge)
        }
    }
